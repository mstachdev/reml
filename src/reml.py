...
def get_default_config():
    """Return a default configuration dictionary. Safe to call when importing."""
    config = {
        "seed": 41,
        "device": "cuda",
        "exp_file": "",
...
def _wandb_log(data, use_wandb=True):
    """Log to wandb only if enabled and available."""
    if use_wandb and _WANDB_AVAILABLE:
        wandb.log(data)


def _wandb_histogram(data, use_wandb=True):
    """Create a wandb Histogram only if enabled and available."""
    if use_wandb and _WANDB_AVAILABLE:
...
def generate_tasks(config):
    ...
    amps = (
        torch.rand(config["n_tasks"], 1) * (amplitude_range[1] - amplitude_range[0])
        + amplitude_range[0]
    )
    phases = (
        torch.rand(config["n_tasks"], 1) * (phase_range[1] - phase_range[0])
        + phase_range[0]
    )
    tasks_data = torch.tensor(
        np.array([X for _ in range(config["n_tasks"])]).astype(np.float32)
    ).float()
    tasks_targets = torch.tensor(
        np.array(
            [[(a * np.sin(x + p)).float() for x in X] for a, p in zip(amps, phases)]
        ).astype(np.float32)
    ).float()
    tasks_info = [
        {
            ...
            "phase_range_upper_bound": phase_range[1],
        }
...
def generate_configs(experiments, base_config):
    configs = []
...
class LayerPool:
    def __init__(
        ...
        num_nodes_per_layer: int = None,
        layers: List[torch.nn.Linear] = None,
    ):
        self.config = config
        self.in_features = (
            in_features if in_features is not None else config["in_features"]
        )
        self.out_features = (
            out_features if out_features is not None else config["out_features"]
        )
        self.num_nodes_per_layer = (
            num_nodes_per_layer
            if num_nodes_per_layer is not None
            else config["n_nodes_per_layer"]
        )

        if layers is None:
            self.layers = [
                torch.nn.Linear(
                    in_features=self.num_nodes_per_layer,
                    out_features=self.num_nodes_per_layer,
                )
                for _ in range(config["n_pool_hidden_layers"])
            ]
        ...
        self.initial_input_layer = copy.deepcopy(
            random.choice([layer for layer in self.layers if layer.in_features == 1])
        )
        self.initial_output_layer = copy.deepcopy(
            random.choice([layer for layer in self.layers if layer.out_features == 1])
        )

    def __str__(self) -> str:
        return f"LayerPool(size={len(self.layers)}, num_nodes_per_layer={self.num_nodes_per_layer})"
...
class InnerNetworkTask(Dataset):
    def __init__(self, data, targets, info, config=None):
        self.data = data
    ...
    def __len__(self):
        if self._config is not None:
            assert len(self.data) == self._config["n_x"], (
                "[ERROR] Length should be the same as n_x."
            )
        return len(self.data)

    def __getitem__(self, index):
        assert self.data[index].dtype == torch.float32, (
            f"[ERROR] Expected type torch.float32, got type: {self.data[index].dtype}"
        )
        assert self.targets[index].dtype == torch.float32, (
            f"[ERROR] Expected type torch.float32, got type: {self.targets[index].dtype}"
        )
        sample = {"x": self.data[index], "y": self.targets[index], "info": self.info}
        return sample
...
class InnerNetwork(gymnasium.Env, torch.nn.Module):
    def __init__(
        ...
        self.data_loader = DataLoader(task, batch_size=self.batch_size, shuffle=shuffle)
        ...
        self.layers = torch.nn.ModuleList(
            [self.initial_input_layer, self.initial_output_layer]
        )
    ...
    def step(self, action: np.int64) -> Tuple[torch.Tensor, float, bool, dict]:
        assert action.shape == (), (
            f"[ERROR] Expected action shape () for scalar {self.action_space.n}, got: {action.shape}"
        )
        assert action.dtype == np.int64, (
            f"[ERROR] Expected np.int64 dtype, got: {action.dtype}"
        )
        ...
        self.update(action)
        termination = (
            False if len(self.layers) < self.config["n_layers_per_network"] else True
        )
        if termination:
            self.train_inner_network()
        else:
        ...
        if throw_exception:
            ...
            try:
                ...
                self.data_loader = DataLoader(
                    self.task, batch_size=self.batch_size, shuffle=self.shuffle
                )
                self.data_iter = iter(self.data_loader)
                batch = next(self.data_iter)
            finally:
    ...
    def update(self, action: np.int64) -> None:
        ...
        # not first or last step and hidden layer and not already in network
            ...
            and new_layer.in_features != 1
            and new_layer.out_features != 1
            and len(self.layers) < self.config["n_layers_per_network"]
        ):
    ...
    def build_state(self) -> np.ndarray:
        task_info = torch.tensor(
            [self.task.info["amp"], self.task.info["phase_shift"]]
        ).squeeze()
        loss = torch.Tensor([self.curr["loss"]])
        yhat_scale = torch.Tensor(
            [torch.Tensor(torch.max(torch.abs(self.curr["y_hat"]))).detach().item()]
        )
        one_hot_layers = torch.tensor(
            np.array(
                [
                    1 if self.layer_pool.layers[i] in self.layers else 0
                    for i in range(len(self.layer_pool.layers))
                ]
            )
        )
        layer_indices = [
            index + 1 for index in self.layers_pool_indices.copy()
        ]  # 0 bump by 1 to avoid 0th index layer being 0 since padding 0s too
        while len(layer_indices) < self.config["n_layers_per_network"]:
    ...
    def reward(self) -> torch.Tensor:
        ...
        if self.curr["action_type"] == InnerNetworkAction.ERROR:
            ...
            reward = -(
                (self.curr["loss"] - self.task_min_loss + epsilon)
                / (self.task_max_loss - self.task_min_loss + epsilon)
            )
    ...
    def log(self):
        ...
        if self.use_wandb and _WANDB_AVAILABLE:
            ...
            wandb.log(
                {
                    f"pool_indices_task{task_num}_per_step": wandb.Histogram(
                        torch.tensor(self.layers_pool_indices)
                    )
                }
            )
    ...
    def train(self):
        ...
        for epoch in range(self.config["epochs"]):
            ...
            for i, task in enumerate(self.tasks):
                ...
                self.config["task_max_loss"][self.task.info["i"]] = max(
                    local_max_loss, self.config["task_max_loss"][self.task.info["i"]]
                )
    ...
    def __init__(self, config):
        ...
        self.layers = torch.nn.ModuleList(
            [
                ...
                torch.nn.Linear(
                    config["n_nodes_per_layer"], config["n_nodes_per_layer"]
                ),
                torch.nn.Linear(
                    config["n_nodes_per_layer"], config["n_nodes_per_layer"]
                ),
...
def run(config, experiments=None, seed=41):
    ...
    if experiments:
        ...
        tasks = [
            InnerNetworkTask(
                data=data[i], targets=targets[i], info=info[i], config=config
            )
        ...
        configs = generate_configs(experiments, config)
        exp_rewards_per_episode_across_tasks = defaultdict(
            lambda: list()
        )  # { exp: [ [[episode reward], ...],] ...tasks}
        exp_errors_per_episode_across_tasks = defaultdict(lambda: list())
        for exp, exp_config in zip(experiments, configs):
            pool = LayerPool(config=exp_config)
            reml = REML(
                tasks=training_tasks, layer_pool=pool, run=exp, config=exp_config
            )
            reml.train()
            ...
            for task in tasks:
                exp_rewards_per_episode_across_tasks[exp_string].append(
                    reml.task_rewards_per_episode[str(task.info["i"])]
                )
                exp_errors_per_episode_across_tasks[exp_string].append(
                    reml.task_errors_per_episode[str(task.info["i"])]
                )

        with open(
            os.path.join(path, f"exp_rewards_per_episode_across_tasks"), "w"
        ) as json_file:
            json.dump(exp_rewards_per_episode_across_tasks, json_file, indent=4)

        with open(
            os.path.join(path, f"exp_errors_per_episode_across_tasks"), "w"
        ) as json_file:
        ...
        for run_num in range(1, config["n_runs"] + 1):
            ...
            set_seed(seed, config_copy)  # new seed per task

            pool = LayerPool(config=config_copy)
            reml = REML(
                tasks=training_tasks, layer_pool=pool, run=run_num, config=config_copy
            )
            reml.train()
            ...
            # save return, loss, and error data across runs
            for task in tasks:
                task_rewards_per_episode_across_runs[str(task.info["i"])].append(
                    reml.task_rewards_per_episode[str(task.info["i"])]
                )
                task_losses_per_episode_across_runs[str(task.info["i"])].append(
                    reml.task_losses_per_episode[str(task.info["i"])]
                )
                task_errors_per_epsiode_across_runs[str(task.info["i"])].append(
                    reml.task_errors_per_episode[str(task.info["i"])]
                )
        ...
        with open(
            os.path.join(path, f"task_rewards_per_episode_across_runs"), "w"
        ) as json_file:
            json.dump(task_rewards_per_episode_across_runs, json_file, indent=4)
        with open(
            os.path.join(path, f"task_losses_per_episode_across_runs"), "w"
        ) as json_file:
            json.dump(task_losses_per_episode_across_runs, json_file, indent=4)
        with open(
            os.path.join(path, f"task_errors_per_episode_across_runs"), "w"
        ) as json_file:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--exp_file",
        type=str,
        default=default_config["exp_file"],
        help="Path to JSON file containing list of dictionaries with experiments",
    )
    parser.add_argument(
        "--n_runs",
        type=int,
        default=default_config["n_runs"],
        help="Number of runs",
        required=False,
    )
    parser.add_argument(
        "--n_tasks",
        type=int,
        default=default_config["n_tasks"],
    ...
    parser.add_argument(
        "--sb3_model",
        type=str,
        default=default_config["sb3_model"],
        help="SB3 model to use",
        required=False,
    )
    parser.add_argument(
        "--data_dir",
    ...
    parser.add_argument(
        "--wandb_tag",
        type=str,
        default=default_config["wandb_tag"],
        help="Name of run on wandb",
        required=False,
    )
    parser.add_argument(
        "--meta_recurrent_n_steps",
        type=int,
        default=default_config["meta_recurrent_n_steps"],
        help="N steps(horizon)",
        required=False,
    )
    parser.add_argument(
        "--no_wandb",
    ...
    config = {
        key: getattr(args, key, default_value)
